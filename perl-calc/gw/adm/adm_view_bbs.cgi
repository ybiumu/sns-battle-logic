...
my $board_descr = $board->getBoardDescr($board_id);

my $board_type_id = $board_descr->{"board_type_id"};
...
else
{
    ...
    foreach my $entry ( @{$entries} )
    {
        ...
        $msg =~ s/\n/<br \/>/g;
        $msg =~ s/ /&nbsp;/g;
        $msg =~ s/&gt;&gt;(\d+)/<a href="adm_view_bbs.cgi?guid=ON&t=$board_type_id&oid=$target_user_id&tid=$1">&gt;&gt;$1<\/a>/g;
        $out->{RESULT} .= sprintf
            "<hr />\n[<a href='adm_view_bbs.cgi?guid=ON&bw=1&t=%s&oid=%s&tid=%s'>%s</a>]<a href='mypage.cgi?user_id=%s&guid=ON'>%s</a><br />(%s)<br />%s",
            $board_type_id,
            $target_user_id,
            $entry->{thread_id},
            $entry->{thread_id},
    ...
    if ( ! $thread_id )
    {
        ...
        else
        {
            ...
            {
                $out->{RESULT} .= sprintf
                    '<a href="adm_view_bbs.cgi?guid=ON&t=%s&oid=%s&p=%s">‘O‚Ö</a>',
                    $board_type_id,
                    $target_user_id,
                    ,$page_id - 1;
            }
            ...
            {
                $out->{RESULT} .= sprintf
                    '|<a href="adm_view_bbs.cgi?guid=ON&t=%s&oid=%s&p=%s">ŽŸ‚Ö</a>',
                    $board_type_id,
                    $target_user_id,
                    ,$page_id + 1;
            }
    ...
    if ( $thread_id )
    {
        $out->{SRC_PAGE} = sprintf( "adm_view_bbs.cgi?guid=ON&t=%s&p=%s&oid=%s", $board_type_id,$page_id, $target_user_id );
    }
