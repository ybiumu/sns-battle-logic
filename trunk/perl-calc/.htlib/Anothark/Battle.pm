...
my $target_template = '<div style="text-align:%s">ÅÀ%s</div>';
...
my $effect_str = {
    ...
    1 => {0 => "âÒïú",   1 => "¿ﬁ“∞ºﬁ"},
    2 => {0 => "",       1 => ""},
    3 => {0 => "„©Çédä|ÇØÇΩ" , 1 => "„©Çédä|ÇØÇΩ"},
    4 => {0 => "éÙÊfÇédä|ÇØÇΩ" , 1 => "éÙÊfÇédä|ÇØÇΩ"},
    5 => {0 => "", 1 => ""},
    6 => {0 => "", 1 => ""} 
};
...
sub resolveDamages
{
    ...
    if ( $dmg == 0 )
    {
        $class->getTurnText()->[$turn] .= sprintf(
                                                ...
                                                sprintf($effect_str_template, "å¯â Ç»Çµ")
                                          );
    }
    else
    {
    ...
    if( not $target->isLiving() )
    {
        $class->getTurnText()->[$turn] .= sprintf(
                                            $effect_template,
                                            $symbol->{$char->getSide()}->{align}, "Å@ÅÀÅÀì|ÇÍÇΩ")
    }
}
...
#    my $char  = shift;
#
#    my $side  = $char->getSide(); # ¿∞πﬁØƒÇ™èäëÆÇ∑ÇÈSide
#    my $target_type = shift; #1 enemy ,2 self,3 all # ¿∞πﬁØƒÇ©ÇÁå©ÇΩëäëŒìI
#    # 1 ÇÃéû(enemy)
#    #   1. side -> e
#    #    e is true
#    #    p is false
...
#                $char->getName(),
#            );
#warn "Name: [".$char->getName()."] ÇÃ ¿∞πﬁØ√®›∏ﬁ";
## cmd start
#            # Cmd
#            $class->getTurnText()->[$turn] .= sprintf(
#                $cmd_template,
...
#                                                        $effect_template,
#                                                        $symbol->{$char->getSide()}->{align},
#                                                        sprintf($effect_str_template, $char->getCmd()->[$turn]->getBaseElementName(), ( $dmg . "¿ﬁ“∞ºﬁ"))
#                                                  );
#                $target->Damage( $dmg );
#                if( not $target->isLiving() )
#                {
...
#            else
#            {
#                $class->getTurnText()->[$turn] .= sprintf($effect_template, $symbol->{$char->getSide()}->{align},sprintf( $effect_str_template, "ìÕÇ©Ç»Ç¢"));
#            }
#
#}
#            # Post Effect Check
...
#                warn "range type 2";
            ...
            {
                $class->getTurnText()->[$turn] .= sprintf("Ç»ÇÒÇ©ÉGÉâÅ[");
            }
...
#    return getRealDamage( $skill->getSkillRate(), $to->gDef()->cv(), 0 ); ëœê´ÇÕâºÇÃíl
}
