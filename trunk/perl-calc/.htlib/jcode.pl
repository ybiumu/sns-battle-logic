...
our    $re_sjis_c    = '[\201-\237\340-\374][\100-\176\200-\374]';
...
#    j_out_log("[STR2] " . $s );
#    j_out_log("$re_sjis_c");
#    my @key_map = ($$s =~ m/($re_sjis_c)/go);
#    j_out_log("map");
#    j_out_log("[maps]" . join("/",@key_map));
...
sub j_out_log
{
    open LOG, ">>../../.htlog/jcode.log" or die;
    printf LOG "[%s][JCODE] %s\n", scalar(localtime()), join("",@_);
    close LOG;
}
